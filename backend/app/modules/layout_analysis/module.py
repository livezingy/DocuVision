...
FROZEN NOTICE:
- This module is intentionally frozen and is not on the active server execution path.
- The current production path uses app.services.layout_service.LayoutService.
- Keep this file for compatibility/reference only unless a full module-registry migration is planned.
"""
...
_FROZEN_MODULE_NOTICE = (
    "[FROZEN_MODULE] LayoutAnalysisModule is frozen and not used by the active server path. "
    "Active path: app.services.layout_service.LayoutService"
)
...
class LayoutAnalysisModule(BaseModule):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._frozen_notice_emitted = False

    def initialize(self, config: Optional[Dict[str, Any]] = None) -> bool:
        """Initialize module with engines"""
        if not self._frozen_notice_emitted:
            logger.warning(_FROZEN_MODULE_NOTICE)
            self._frozen_notice_emitted = True
