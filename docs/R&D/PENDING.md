...
按标题自标统计：**已结保留记录 2 条**（P-014 ✅ 已结、P-019 已结保留记录），**已裁决并落地且已验证 1 条**（P-021：2026-09-24 裁决 + 落地入库 → 2026-09-25 round3 云端复测通过，§10.11 门禁 FAIL→PASS），**已裁决并落地 1 条**（P-023：2026-09-25 登记同日落地 R1–R3），其余 **13 条待裁决**（其中 P-024 为长对话「开新对话」提议无触发点，2026-09-25 登记，待裁决）。
...
- **现行控制**：**无**（这正是本条目要修的）；在此之前，长对话"该不该重开"仍只依赖用户提醒。
- **触发条件**：任何长对话（≥1 次压缩、或触及四条代理任一）收尾 / 换阶段时；改 `constraints.md` 的会话管理条时同查本条目。
- **状态（2026-09-25）**：**已登记，待裁决**（本轮仅登记：未改 kernel、未改脚本、未动 CI）。
