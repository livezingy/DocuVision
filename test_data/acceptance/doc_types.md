...
| 发票 | `invoice` | true | `test_data/testfiles/invoices/invoice_sample_01.pdf`；`test_data/testfiles/invoices/sample-invoice.png` | `DOCUVISION_RUN_KIE_ACCEPTANCE=1 pytest backend/tests/test_kie_acceptance_baseline.py -q`（重）；或本机对同一文件 `analyze` + KIE | `view.fields` / `kie_fields` 关键键；导出 JSON |
| 收据 | `receipt` | true | `test_data/testfiles/invoices/receipt-invoice-like.png`（收据类占位） | 同上 opt-in smoke | 字段稳定性 |
