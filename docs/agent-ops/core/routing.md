...
> 生效前提：`feature/v1.8.2` 的 main.py 拆分完成（`backend/app/routers/` 存在）；拆分前本规范为「目标态」，`lint_routes.py` 自动跳过未拆分状态。

## 目标
把 2600+ 行的 `main.py` 收敛为 ~200 行装配层；路由全部进 `backend/app/routers/`，用机器可判规则锁死风格，使不同 Agent 后续新增路由时产出同构代码。
...
5. **更新 OpenAPI 快照**：新增/删除路由改变 `app.openapi()` → 全量快照测试 additive 更新 baseline 并重新落盘（云测）；未同步更新快照 = 红。
