...
职责�?
- 基于PageFeatureAnalyzer的分析结果判断表格类�?
- 实现多层级判断逻辑（快速过滤、对齐检查、统计特征）
- 返回'bordered'�?unbordered'
...
class TableTypeClassifier:
    ...
    def predict_table_type(self) -> Literal['bordered', 'unbordered']:
        ...
        # 计算关键指标
        line_concentration = main_region_lines / len(all_lines)  # 线条集中�?
        area_ratio = main_region_area / page_area  # 区域集中�?
    ...
    def _quick_alignment_check(self, lines, direction):
        ...
        # 简单分组：相邻坐标差小于tolerance则归为一�?
        tolerance = 3  # 固定tolerance，快速判�?
    ...
    def _calculate_dynamic_threshold(self):
        ...
        原理�?
        - 页面越大，可能容纳的表格越大，阈值相应提�?
        - 基于A4页面�?95x842pt）作为基准，阈值设�?0
        - 其他页面按面积比例缩�?
