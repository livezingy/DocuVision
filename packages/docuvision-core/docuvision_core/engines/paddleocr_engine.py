...
封装PaddleOCR的OCR和PP-Structure表格检测功�?
支持文本识别、表格检测和结构识别
"""
...
class PaddleOCREngine(BaseOCREngine, BaseDetectionEngine):
    ...
    def load_models(self, **kwargs) -> bool:
        ...
        # 首先检查是否可以使�?PPStructureV3 (PaddleOCR 3.x)
        ...
        if ppstructure_v3_available:
            ...
            try:
                ...
                # PPStructureV3 的参数可能因版本而异，优先使用最小化配置以减少模型下�?
                # 禁用不必要的功能以加快初始化速度并减少内存占用（特别是Streamlit Cloud环境�?
                init_success = False
                last_error = None
                
                # 尝试1: 最小化配置 - 只启用表格检测和识别，禁用所有其他功�?
                ...
                if "DependencyError" in str(type(e)) or "paddlex" in str(e).lower():
                    ...
                    return self._load_legacy_ppstructure(kwargs)
                # �?3.x 版本中，PPStructure 不存在，所以不需要回退
                return False
        else:
            # 尝试旧版本的 PPStructure
            return self._load_legacy_ppstructure(kwargs)
    ...
    def detect_tables(self, image: Image.Image, **kwargs) -> List[Dict]:
        ...
        try:
            ...
            if self._is_ppstructure_v3:
                # PPStructureV3 使用 predict() 方法，可以直接接�?PIL Image �?numpy array
                # 优先使用 PIL Image，如果失败则尝试 numpy array
                try:
                    # 直接使用 PIL Image
                    results = self._structure_engine.predict(image)
                except (TypeError, AttributeError):
                    # 如果 PIL Image 不支持，转换�?numpy array
                    # PPStructureV3 期望 BGR 格式�?numpy array
                    import cv2
                    img_array = np.array(image)
                    # PIL Image �?RGB，需要转换为 BGR
                ...
                # 旧版�?PPStructure 使用 __call__ 方法
                # 注意：PPStructure的__call__方法不接受layout参数，直接传入图像数组即�?
    ...
    def recognize_structure(self, image: Image.Image, table_bbox: Optional[List] = None, **kwargs) -> Dict:
        ...
        try:
            ...
            if self._is_ppstructure_v3:
                ...
                result = {
                    'html': html_content or markdown_content,  # 使用 HTML �?markdown
                    'cells': cells,
