...
/** Semantic text blocks: prefer the flat /blocks payload, else view pages, else result. */
function resolveSemanticTextBlocks(blocksData, result) {
    if (blocksData) {
        return blocksData.blocks.filter(b => {
    ...
    return fromView.length ? fromView : (result.semantic_text_blocks || []);
}

/** Layout elements whose type is text-like - the fallback when no semantic blocks exist. */
function resolveLayoutTextElements(result) {
...
/** Normalised text elements: semantic blocks win, layout elements are the fallback. */
function resolveTextElements(result, semanticTextBlocks) {
    const layoutTextElements = resolveLayoutTextElements(result);
    ...
    return semanticElements.length > 0 ? semanticElements : layoutTextElements.map(el => ({
...
/** Longest normalised OCR paragraph per block, used to repair collapsed text. */
function buildOcrCandidates(textBlocks) {
    return textBlocks
...
/** Collapse-aware grouping: page -> Azure type label -> items. */
function buildPageMap(textElements, ocrCandidates) {
...
/** Page-grouped Text panel markup (type order pinned for display stability). */
function renderTextElementsByPage(textElements, ocrCandidates) {
    ...
    const pageMap = buildPageMap(textElements, ocrCandidates);
...
/** Flat first-50 text blocks markup (no semantic / layout elements available). */
function renderFlatTextBlocks(textBlocks) {
    let html = '';
...
/** Whole-document text markup (first 10000 chars, then an ellipsis). */
function renderFullText(fullText) {
...
export function updateContentText(result) {
    ...
    const semanticTextBlocks = resolveSemanticTextBlocks(blocksData, result);
    const textElements = resolveTextElements(result, semanticTextBlocks);
    const ocrCandidates = buildOcrCandidates(textBlocks);
    const fullText = result.full_text || '';

    let html = '';

    if (textElements.length > 0) {
        html = renderTextElementsByPage(textElements, ocrCandidates);
    } else if (textBlocks.length > 0) {
        html = renderFlatTextBlocks(textBlocks);
    } else if (fullText) {
        html = renderFullText(fullText);
    }
