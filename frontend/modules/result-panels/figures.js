...
/** Figure ids the backend flagged (split-figure integrity warnings). */
function collectFigureWarningIds(warnings) {
...
/** No-crop fallback: headers-only cards from layout figure elements, or an empty state. */
function renderLayoutFigureCards(layoutElements, figuresExport) {
    ...
    if (layoutFigs.length === 0) {
        ...
        const msg = errs.length
            ...
            : 'No figures detected';
        return `<div class="empty-state" style="padding: 40px; text-align: center; color: var(--text-tertiary);">${msg}</div>`;
    }
...
/** Crop export items mapped to the card shape (merged / warned flags resolved). */
function mapCropItems(cropItems, warningIds) {
    return cropItems.map((item, index) => ({
...
/** Split-figure warning banner + merged-crop switcher ('' when there is nothing to warn about). */
function renderFigureWarningsBanner(warnings) {
    const splitWarnings = warnings.filter(function (w) {
        return w.kind === 'possible_vertical_split' || w.kind === 'possible_horizontal_split';
    });
    if (!(splitWarnings.length > 0 || window.mergedFigures.length > 0)) return '';
...
/** Prev / next figure navigation ('' unless more than one original crop exists). */
function renderFigureNavigation() {
    if (!(window.currentFigures.length > 1)) return '';
...
/** Lazy auth-loaded crop image of a card. */
...
/** Repaint the visible card: merged crop when browsing a merge, else the paginated original. */
function paintVisibleFigure(contentFiguresList) {
...
/** Prev / next pagination bindings (only meaningful with more than one original crop). */
function bindFigurePagination(contentFiguresList) {
    if (!(window.currentFigures.length > 1)) return;
    ...
    if (prevBtn) {
        prevBtn.addEventListener('click', () => {
            if (window.currentFigureIndex > 0) {
                ...
                paintVisibleFigure(contentFiguresList);
...
/** Merged-view toggle, first crop image load and pagination bindings. */
function bindFigureCardInteractions(contentFiguresList) {
    loadFigureCardImage(contentFiguresList.querySelector('.figure-card'));
    ...
    bindFigurePagination(contentFiguresList);
}
...
export function updateContentFigures(result) {
    ...
    const warningIds = collectFigureWarningIds(warnings);
    ...
    if (!hasCrops) {
        contentFiguresList.innerHTML = renderLayoutFigureCards(layoutElements, figuresExport);
        return;
    }
    ...
    const allMapped = mapCropItems(cropItems, warningIds);
    ...
    let html = '';
    html += renderFigureWarningsBanner(warnings);
    html += renderFigureNavigation();
    html += renderFigureCard(window.currentFigures[0], 0, window.currentFigures.length);
    contentFiguresList.innerHTML = html;

    bindFigureCardInteractions(contentFiguresList);
}
