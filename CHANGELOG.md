...
- **P-023 — test stub scope is now both a rule and a gate** (2026-09-25, born from a P-021 CI run): that run reddened
  Phase A CI in a file the branch never touched, because a **module-level** `sys.modules["paddle"]` stub is installed at
  *collection* time - it outlived the test that needed it and stopped `test_table_template_analyze.py`'s
  `pytest.importorskip("paddle")` gate from skipping, which then died on the `fastapi` import that guard was only
  *indirectly* protecting. The cause was a rule gap rather than context: the pytest-boundary rule only asked "does your
  test need Paddle/GPU?", never "does it fake Paddle for someone else?", and both existing precedents taught module-level
  stubbing without noting that it is safe only outside the Phase A list. Now: kernel `docs/agent-ops/core/testing.md`
  states the criterion (a file in the Phase A list must scope its stub - `monkeypatch.setitem` in a fixture, or
  `unittest.mock.patch.dict` around a module load) and names both failure modes (a wrong red; worse, silent loss of
  coverage when a gate only decides *whether to run*); `scripts/test_registry_audit.py` gains **check 5**
...
- Kernel `docs/agent-ops/core/testing.md` also gains a validation obligation tied to check 5's finding: when a test **joins
  the Phase A CI list**, the local verification must run that command's **whole file list**, not a single file -
  cross-file contamination only shows up in a full run (the first version of that test was green in isolation and only
  failed in CI). The generated `.cursor/rules/004-project.mdc` / `.codebuddy/rules/004-project.md` copies were
  regenerated by `scripts/sync_agent_rules.py`, not hand-edited.
- **P-022 promoted out of PENDING and into a living doc** (2026-09-25, via the decision log's own "结论确认 → 晋升
  `docs/architecture/` → 从本清单移除" path): the tombstone-gate spec now lives in
  `docs/architecture/doc-governance.md` - rule text, scope, exemptions, the ALLOWLIST cap, the two deliberate 不做 items
  and the three honestly-registered 口径边界 - indexed from `docs/README.md`, registered in `module-map.md` §5 as
  **DOC-1** / **DOC-2**, and assigned an owning row in `docs/agent-ops/doc-sync-ownership.md`. The token list is
  deliberately **not** copied into the doc: `RETIRED` / `SCAN_GLOBS` / `EXEMPT` / `ALLOWLIST` stay in
  `scripts/docs_refs_audit.py` as the single source - both to avoid a second source of truth and because
  `docs/architecture/**` is itself scanned, so copying the literals would trip the gate on its own specification.
  `docs_refs_audit.py`'s docstring now points at the living doc rather than the PENDING id. PENDING drops to 16 entries
  (13 待裁决) and records P-022 on its history line, together with the index omission noticed earlier.
- **P-024 — the "propose a new chat" rule gains forced trigger points instead of pure introspection** (2026-09-25): the
  four observable proxies in `constraints.md` (compaction event / repeated file read >= 2 / consecutive tool failures >= 2 /
  self-reversal) never fired in practice because nothing *forces* the evaluation - the rule was a conditional statement
  whose execution depended on remembering to check, and compaction deletes the very ledger the counting needs. Kernel
  `constraints.md` (沟通方式节) now adds two mandatory checkpoints: (1) an input *recognizable* as a compaction/handoff
  summary counts as the trigger already fired - recognizability is **not** guaranteed by the platform, so the clause is
  conditional and must be verified at the next compaction with the result recorded back into PENDING P-024; (2) every
  commit/PR and every CHANGELOG/PENDING write carries one line
  `Session-check: rereads=<n> failures=<n> reversals=<n> compaction=<0|1> -> propose=<yes|no>`, **zero counts included**,
  so skipping leaves a visible blank in the commit message / PR body where a reviewer can spot it. Deliberately **no CI
  machine check**: `audit_agent_ops.py` runs on a checkout and cannot read PR bodies, and a shallow CI clone usually
  cannot see the PR's own commits (a machine check would need a fetch-depth change or a push-only scan - both CI-config
  red lines). Per the user's ruling, option (c) (persisted session ledger) is parked and option (d) is downgraded to the
  commit-message/PR-body field above. Generated copies `.cursor/rules/001-general.mdc` / `.codebuddy/rules/001-general.md`
  were regenerated by `scripts/sync_agent_rules.py`, not hand-edited.
- **PENDING clear-out batch** (2026-09-25): four entries whose conclusions had long been carried by living docs were
  promoted out and removed via the decision log's own "结论确认 → 晋升 → 移除" path - **P-007** (KIE meta row gate,
  conclusion lives in `docs/architecture/v1.9-roadmap.md` §Scope S1), **P-010** (frontend style linter, conclusion in
  `docs/agent-ops/operations.md` §CI 成本与配额 + the ESLint config itself), **P-013** (service-layer owning-doc split,
  conclusion in `docs/agent-ops/doc-sync-ownership.md` 主表+脚注 2) and **P-016** (inline-handler ReferenceError, root
  cause + fix in code, reusable conclusion recorded in kernel `frontend.md`'s known gaps). Detailed evidence remains in
  earlier CHANGELOG entries and git history; all four are recorded on PENDING's history line. **P-011** (CI main-only
  triggering) stays as a closed-but-kept record because its 2026-09-17 paths ruling is the reference for re-narrowing
  `pull_request.paths` if the repo ever goes private - nothing else carries that list. PENDING drops 17 → 13 entries
  (7 真待裁决); **P-018**'s status line was corrected in the same batch: its P0 harness and P1 weak-GT synthesis already
  shipped via P-020 (2026-09-24), so the next open decision is the P2 table-structure GT reconciliation dataset choice
  (PubTabNet / FinTabNet / WTW). Docs-only; audit 0 error / 0 warning unchanged.
- **P-025 — R&D promotion was an obligation with no trigger point, so it now has a gate and a checkpoint** (2026-09-26):
  the kernel said "promote stable R&D conclusions into `docs/architecture/`" and nothing checked it - the six audit checks
  read neither the R&D folder nor the decision log, and `docs_refs_audit.py` even exempts `docs/R&D/**` from the DOC-1 scan.
  The structural half cannot be fixed: `.gitignore` lets only `README.md` and `PENDING.md` out of that folder, so CI never
  has the other files in its checkout. The half that *can* be checked was worth checking. **(a) gate DOC-3**: every entry in
  `docs/R&D/PENDING.md` must carry one `> status: <open|decided|landed|retained> · since: YYYY-MM-DD` line (single source of
  truth; the header keeps an id index and **no** hand-written state counts - that copy had already drifted, counting five
  closed entries as "待裁决" before the clear-out), and a `landed` entry older than `PENDING_STALE_DAYS = 90` raises a WARN
  asking to promote it or to reclassify it as `retained` with a stated reason. Missing / malformed / future-dated metadata,
  duplicate headings and a header index that disagrees with the `### P-xxx` headings are ERRORs (fail-closed, same shape as
  A6 orphan staleness). The check lives in `scripts/docs_refs_audit.py` and is imported by `audit_agent_ops.py`, so the
  existing required `agent-ops-audit` check runs it - **no CI configuration change**, and no new tracked file under
  `scripts/` (which would have needed another `.gitignore` exception from the user's never-track list). Regression:
  `docs_refs_audit.py --selftest` 10 -> 23, `audit_agent_ops.py --selftest` 31 -> 44. **(b) checkpoint**: kernel
  `doc-sync.md` now requires every PENDING / CHANGELOG write to answer "did anything reach promote-or-remove this round?"
  with one `Promotion-check: <n> eligible (P-xxx…) -> promoted|deferred` line in the commit message / PR body (zero
  included), the same blank-leaves-a-trace shape as `Session-check`; the generated copies `.cursor/rules/009-doc-sync.mdc` /
  `.codebuddy/rules/009-doc-sync.md` were regenerated by `scripts/sync_agent_rules.py`, not hand-edited. **Two KPIs were
  unmeasurable**: `operations.md`'s "记忆回流及时率" cited `MEMORY.md`, which exists nowhere in the repo, and
  "待决决策滞留" had no measurement at all - both now derive from the PENDING metadata (DOC-3), and the 巡检 list no longer
...
- **Two `kind: full` test files no longer leak their stubs** (P-023; found by the new check 5 on its first real run):
  `backend/tests/test_layout_types.py` and `test_layout_page_skip.py` installed their `paddle` / `cv2` stubs at module
  level, so *any* session that collected the whole directory defeated `test_table_template_analyze.py`'s
  `importorskip("paddle")` gate - meaning the local suite was already red for that reason alone (`1 failed`), with zero
  involvement from the P-021 work. Both now stub inside `unittest.mock.patch.dict(sys.modules, ...)` around the module
...
- **P-008 — docs-and-tests governance gates** (2026-09-17, governance batch):
  ...
  - **CI cost/quota policy recorded** — `docs/agent-ops/operations.md` gains a "CI 成本与配额" section (2026-09-17):
    ...
    before the ESLint block, KIE Phase A 21-27s; after the block landed the first real run took **19s**, the block
    itself costing ~6s (setup-node 1s + `npm ci` 4s on a **cold** npm cache + `eslint .` 1s) - far below the
    30-60s estimate it replaced. GitHub bills per
